...
\n\n## State Management\nThe inputs are automatically stored within the browser's `localStorage`. Data conforms to the specific version of the app.\n\n> **TODO:** Future agents need to implement an automatic migration path whenever `STORAGE_VERSION` is incremented in `app.js`. For now, it drops the old local storage and requests the user to enter their values again.
